...
public class QwenAdapter implements ModelAdapter {
    ...
    public Flux<MessageChunk> query(
        ...
        log.info("调用Qwen模型: {}", profile.getModelName());
        ...
        return webClient.post()
                ...
                .doOnError(error -> log.error("Qwen API调用失败", error));
    }
    ...
    public ModelCapabilities getCapabilities() {
        return ModelCapabilities.builder()
                ...
                .supportsTools(true)
                .supportsVision(profile.getModelName().contains("vl"))
    ...
    private Flux<MessageChunk> parseStreamEvent(String event) {
        if (event.startsWith("data: ")) {
            ...
            try {
                ...
                log.warn("解析Qwen SSE事件失败: {}", jsonData, e);
                return Flux.empty();
            }
        }
        return Flux.empty();
